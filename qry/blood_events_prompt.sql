SELECT
	...
	CE_BLOOD.EVENT_ID AS EVENT_ID,
	TO_CHAR(pi_from_gmt(CE_BLOOD.EVENT_END_DT_TM, (pi_time_zone(1, @Variable('BOUSER')))), 'YYYY-MM-DD"T"HH24:MI:SS') AS EVENT_DATETIME,
	CV_EVENT.DISPLAY AS EVENT,
	CV_CATALOG.DISPLAY AS ORD_CATALOG,
	CV_PRODUCT.DISPLAY AS PRODUCT
FROM
	CE_PRODUCT,
	CLINICAL_EVENT,
	CLINICAL_EVENT CE_BLOOD,
	CODE_VALUE CV_EVENT,
	CODE_VALUE CV_CATALOG,
	CODE_VALUE CV_PRODUCT,
	ENCOUNTER
WHERE
	...
	AND (
		CLINICAL_EVENT.ENCNTR_ID = CE_BLOOD.ENCNTR_ID
		AND ( 
			(CE_BLOOD.EVENT_CD = 240983440 AND CE_BLOOD.CATALOG_CD = 10966863)
			OR CE_BLOOD.EVENT_CD = 33981 
		)
		AND CE_BLOOD.EVENT_CD = CV_EVENT.CODE_VALUE
		AND CE_BLOOD.CATALOG_CD = CV_CATALOG.CODE_VALUE
	)
	AND (
		CE_BLOOD.EVENT_ID = CE_PRODUCT.EVENT_ID(+)
		AND CE_PRODUCT.PRODUCT_CD = CV_PRODUCT.CODE_VALUE(+)
	)
	AND (
